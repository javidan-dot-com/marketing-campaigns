...
from app.schemas.campaign import CampaignCreate, CampaignOutput, CampaignSaveOutput, CampaignUpdate
...
@campaign_router.post("", response_model=CampaignSaveOutput, status_code=status.HTTP_201_CREATED)
...
@campaign_router.put("/{campaign_id}", response_model=CampaignSaveOutput, status_code=status.HTTP_200_OK)
async def update_campaign(campaign_id: int, data: CampaignUpdate, use_case: UpdateCampaignUseCase = Depends(depends_update_campaign_use_case)):
    return await use_case.execute(campaign_id, data.status)